...
static Ref getarg(char c, Ins* i)
{
    switch (c) {
    case '0':
        return i->arg[0];
...
static void emitcopy(Ref r1, Ref r2, int k, Fn* fn, FILE* f)
{
    Ins icp;
...
static void emitf(char* s, Ins* i, Fn* fn, FILE* f)
{
    ...
    case '+':
        ...
        /* fall through */
    case '-':
        assert((!req(i->arg[1], i->to) || req(i->arg[0], i->to)) && "cannot convert to 2-address");
    ...
    Ref:
        ...
        Mem:
            ...
            if (!req(m->base, R))
                ...
                fprintf(f, "%%rip");
            if (!req(m->index, R))
                fprintf(f, ", %%%s, %d", regtoa(m->index.val, SLong), m->scale);
            fputc(')', f);
            break;
        case RCon:
            fputc('$', f);
...
static void emitins(Ins i, Fn* fn, FILE* f)
{
    ...
    Table:
        ...
        for (o = 0;; o++) {
            ...
            if (omap[o].op == i.op)
                if (omap[o].cls == i.cls || (omap[o].cls == Ki && KBASE(i.cls) == 0) || (omap[o].cls == Ka))
                    break;
        }
    ...
    case Oneg:
        ...
        if (KBASE(i.cls) == 0)
            ...
            fprintf(f, "\txorp%c %sfp%d(%%rip), %%%s\n", "xxsd"[i.cls], T.asloc, stashbits(negmask[i.cls], 16), regtoa(i.to.val, SLong));
        break;
    case Odiv:
    ...
    case Ocopy:
        ...
        if (req(i.to, i.arg[0]))
            break;
        t0 = rtype(i.arg[0]);
        if (i.cls == Kl && t0 == RCon && fn->con[i.arg[0].val].type == CBits) {
        ...
        if (isreg(i.to) && t0 == RCon && fn->con[i.arg[0].val].type == CAddr) {
        ...
        break;
    case Oaddr:
        if (!T.apple && rtype(i.arg[0]) == RCon && fn->con[i.arg[0].val].sym.type == SThr) {
            ...
            fprintf(f, "\tleaq %s%s@tpoff", sym[0] == '"' ? "" : T.assym, sym);
            if (con->bits.i)
                fprintf(f, "%+" PRId64, con->bits.i);
