...
static int iscon(Con* c, int w, uint64_t k)
{
    if (c->type != CBits)
        return 0;
    if (w)
...
static int latmerge(int v, int m) { return m == Top ? v : (v == Top || v == m) ? m : Bot; }

static void update(int t, int m, Fn* fn)
{
    Tmp* tmp;
    uint u;
...
static int deadedge(int s, int d)
{
    Edge* e;
...
static void visitphi(Phi* p, int n, Fn* fn)
{
    int v;
    uint a;
...
static void visitins(Ins* i, Fn* fn)
{
    int v, l, r;
...
static void visitjmp(Blk* b, int n, Fn* fn)
{
    int l;
...
static void initedge(Edge* e, Blk* s)
{
    if (s)
        e->dest = s->id;
    else
...
void fold(Fn* fn)
{
    ...
    for (;;) {
        ...
        if (e) {
            ...
            b->visit++;
            assert(b->jmp.type != Jjmp || !edge[n][0].dead || flowrk == &edge[n][0]);
        } else if (nuse) {
            u = usewrk[--nuse];
            n = u->bid;
            b = fn->rpo[n];
...
static int foldint(Con* res, int op, int w, Con* cl, Con* cr)
{
    union {
        int64_t s;
        uint64_t u;
    ...
    case Oneg:
        ...
        break;
    case Odiv:
        x = w ? l.s / r.s : (int32_t)l.s / (int32_t)r.s;
        break;
    case Orem:
        x = w ? l.s % r.s : (int32_t)l.s % (int32_t)r.s;
        break;
    case Oudiv:
        x = w ? l.u / r.u : (uint32_t)l.u / (uint32_t)r.u;
        break;
    case Ourem:
        x = w ? l.u % r.u : (uint32_t)l.u % (uint32_t)r.u;
        break;
    ...
    case Oxor:
        ...
        break;
    case Osar:
        x = (w ? l.s : (int32_t)l.s) >> (r.u & (31 | w << 5));
        break;
    case Oshr:
        x = (w ? l.u : (uint32_t)l.u) >> (r.u & (31 | w << 5));
        break;
    case Oshl:
        x = l.u << (r.u & (31 | w << 5));
        break;
    ...
    case Oextuw:
        ...
        break;
    case Ostosi:
        x = w ? (int64_t)cl->bits.s : (int32_t)cl->bits.s;
        break;
    case Ostoui:
        x = w ? (uint64_t)cl->bits.s : (uint32_t)cl->bits.s;
        break;
    case Odtosi:
        x = w ? (int64_t)cl->bits.d : (int32_t)cl->bits.d;
        break;
    case Odtoui:
        x = w ? (uint64_t)cl->bits.d : (uint32_t)cl->bits.d;
        break;
    case Ocast:
        x = l.u;
        if (cl->type == CAddr) {
...
static void foldflt(Con* res, int op, int w, Con* cl, Con* cr)
{
    float xs, ls, rs;
    double xd, ld, rd;
    ...
    *res = (Con) { .type = CBits };
    ...
    if (w) {
        ...
        case Omul:
            ...
            break;
        case Oswtof:
            xd = (int32_t)cl->bits.i;
            break;
        case Ouwtof:
            xd = (uint32_t)cl->bits.i;
            break;
        case Osltof:
            xd = (int64_t)cl->bits.i;
            break;
        case Oultof:
            xd = (uint64_t)cl->bits.i;
            break;
        case Oexts:
            xd = cl->bits.s;
            break;
        ...
        case Omul:
            ...
            break;
        case Oswtof:
            xs = (int32_t)cl->bits.i;
            break;
        case Ouwtof:
            xs = (uint32_t)cl->bits.i;
            break;
        case Osltof:
            xs = (int64_t)cl->bits.i;
            break;
        case Oultof:
            xs = (uint64_t)cl->bits.i;
            break;
        case Otruncd:
            xs = cl->bits.d;
            break;
...
static int opfold(int op, int cls, Con* cl, Con* cr, Fn* fn)
{
    Ref r;
    Con c;
