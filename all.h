...
#define R                                                                                                                                                      \
    (Ref) { RTmp, 0 }
#define UNDEF                                                                                                                                                  \
    (Ref) { RCon, 0 } /* represents uninitialized data */
#define CON_Z                                                                                                                                                  \
    (Ref) { RCon, 1 }
#define TMP(x)                                                                                                                                                 \
    (Ref) { RTmp, x }
...
#define MEM(x)                                                                                                                                                 \
    (Ref) { RMem, x }
#define INT(x)                                                                                                                                                 \
    (Ref) { RInt, (x) & 0x1fffffff }

static inline int req(Ref a, Ref b) { return a.type == b.type && a.val == b.val; }
...
static inline int rsval(Ref r) { return ((int)r.val ^ 0x10000000) - 0x10000000; }
...
#define JMPS(X)                                                                                                                                                \
    X(retw)                                                                                                                                                    \
    X(retl) X(rets) X(retd) X(retsb) X(retub) X(retsh) X(retuh) X(retc) X(ret0) X(jmp) X(jnz) X(jfieq) X(jfine) X(jfisge) X(jfisgt) X(jfisle) X(jfislt)        \
        X(jfiuge) X(jfiugt) X(jfiule) X(jfiult) X(jffeq) X(jffge) X(jffgt) X(jffle) X(jfflt) X(jffne) X(jffo) X(jffuo) X(hlt)
#define X(j) J##j,
    JMPS(X)
#undef X
        NJmp
...
struct Dat {
    enum { DStart, DEnd, DB, DH, DW, DL, DZ } type;
    char* name;
    Lnk* lnk;
    union {
        int64_t num;
