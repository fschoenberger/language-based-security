...
static Ref iins(int cls, int op, Ref a0, Ref a1, Loc* l)
{
    Insert* ist;
...
static void cast(Ref* r, int cls, Loc* l)
{
    int cls0;
...
static inline void mask(int cls, Ref* r, bits msk, Loc* l)
{
    cast(r, cls, l);
    *r = iins(cls, Oand, *r, getcon(msk, curf), l);
}

static Ref load(Slice sl, bits msk, Loc* l)
{
    Alias* a;
    Ref r, r1;
    int ld, cls, all;
    ...
    ld = (int[]) { [1] = Oloadub, [2] = Oloaduh, [4] = Oloaduw, [8] = Oload }[sl.sz];
    all = msk == MASK(sl.sz);
    if (all)
        cls = sl.cls;
    else
...
static int killsl(Ref r, Slice sl)
{
    Alias* a;
...
static Ref def(Slice sl, bits msk, Blk* b, Ins* i, Loc* il)
{
    Slice sl1;
    Blk* bp;
    bits msk1, msks;
    ...
    while (i > b->ins) {
        --i;
        if (killsl(i->to, sl) || (i->op == Ocall && escapes(sl.ref, curf)))
            goto Load;
        ld = isload(i->op);
        if (ld) {
            sz = loadsz(i);
    ...
    for (np = 0; np < b->npred; ++np) {
        bp = b->pred[np];
        if (!bp->s2 && il->type != LNoLoad && bp->loop < il->blk->loop)
            l.type = LLoad;
        else
            l.type = LNoLoad;
        l.blk = bp;
...
static int icmp(const void* pa, const void* pb)
{
    Insert *a, *b;
    int c;
