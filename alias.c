...
void getalias(Alias* a, Ref r, Fn* fn)
{
    Con* c;
...
int alias(Ref p, int op, int sp, Ref q, int sq, int* delta, Fn* fn)
{
    Alias ap, aq;
    int ovlap;
    ...
    *delta = ap.offset - aq.offset;
    ...
    if ((ap.type == ACon && aq.type == ACon) || (ap.type == aq.type && ap.base == aq.base)) {
...
static void store(Ref r, int sz, Fn* fn)
{
    Alias* a;
    int64_t off;
    bits m;
