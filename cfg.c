    ...
    *pbd = 0;
    ...
    for (p = bd->phi; p; p = p->link) {
        for (a = 0; p->blk[a] != bs; a++)
            assert(a + 1 < p->narg);
        p->narg--;
        memmove(&p->blk[a], &p->blk[a + 1], sizeof p->blk[0] * (p->narg - a));
        memmove(&p->arg[a], &p->arg[a + 1], sizeof p->arg[0] * (p->narg - a));
    }
    if (bd->npred != 0) {
        for (a = 0; bd->pred[a] != bs; a++)
            assert(a + 1 < bd->npred);
        bd->npred--;
        memmove(&bd->pred[a], &bd->pred[a + 1], sizeof bd->pred[0] * (bd->npred - a));
    }
}

static void addpred(Blk* bp, Blk* bc)
{
    if (!bc->pred) {
...
static int rporec(Blk* b, uint x)
{
    Blk *s1, *s2;
...
static Blk* inter(Blk* b1, Blk* b2)
{
    Blk* bt;
...
void filldom(Fn* fn)
{
    ...
    do {
        ...
        for (n = 1; n < fn->nblk; n++) {
            ...
            d = 0;
            for (p = 0; p < b->npred; p++)
                if (b->pred[p]->idom || b->pred[p] == fn->start)
                    d = inter(d, b->pred[p]);
            if (d != b->idom) {
                ch++;
                b->idom = d;
...
int dom(Blk* b1, Blk* b2) { return b1 == b2 || sdom(b1, b2); }

static void addfron(Blk* a, Blk* b)
{
    uint n;
...
static void loopmark(Blk* hd, Blk* b, void f(Blk*, Blk*))
{
    uint p;
...
static void uffind(Blk** pb, Blk** uf)
{
    Blk** pb1;
