...
static int iscon(Ref r, int64_t bits, Fn* fn) { return rtype(r) == RCon && fn->con[r.val].type == CBits && fn->con[r.val].bits.i == bits; }

static int iscopy(Ins* i, Ref r, Fn* fn)
{
    static bits extcpy[] = {
        [WFull] = 0,
...
static Ref copyof(Ref r, Ref* cpy)
{
 ...
 */
static void phisimpl(Phi* p, Ref r, Ref* cpy, Use*** pstk, BSet* ts, BSet* as, Fn* fn)
{
    Use **stk, *u, *u1;
    uint nstk, a;
    int t;
...
static void subst(Ref* pr, Ref* cpy)
{
...
void copy(Fn* fn)
{
    ...
    for (n = 0; n < fn->nblk; n++) {
        ...
        for (p = b->phi; p; p = p->link) {
            ...
            assert(!req(r, R));
            if (rtype(r) == RTmp && !dom(fn->rpo[fn->tmp[r.val].bid], b))
                cpy[p->to.val] = p->to;
            else if (eq == p->narg)
                cpy[p->to.val] = r;
            else {
    ...
    if (debug['C']) {
        ...
        for (t = Tmp0; t < fn->ntmp; t++) {
            if (req(cpy[t], R)) {
                fprintf(stderr, "\n%10s not seen!", fn->tmp[t].name);
            } else if (!req(cpy[t], TMP(t))) {
                fprintf(stderr, "\n%10s copy of ", fn->tmp[t].name);
