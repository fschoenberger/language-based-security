        ...
        *i = (Ins) { .op = Onop };
        ...
        for (u = t->use; u != ue; u++) {
            ...
            if (isstore(l->op)) {
                ...
                if (k == -1)
                    err("slot %%%s is read but never stored to", fn->tmp[l->arg[0].val].name);
...
static inline int rin(Range r, int n) { return r.a <= n && n < r.b; }

static inline int rovlap(Range r0, Range r1) { return r0.b && r1.b && r0.a < r1.b && r1.a < r0.b; }

static void radd(Range* r, int n)
{
    if (!r->b)
        *r = (Range) { n, n + 1 };
    else if (n < r->a)
...
static int slot(Slot** ps, int64_t* off, Ref r, Fn* fn, Slot* sl)
{
    Alias a;
    Tmp* t;
...
static void load(Ref r, bits x, int ip, Fn* fn, Slot* sl)
{
    int64_t off;
    Slot* s;
...
static void store(Ref r, bits x, int ip, Ins* i, Fn* fn, Slot* sl)
{
    int64_t off;
    Slot* s;
...
static int scmp(const void* pa, const void* pb)
{
    Slot *a, *b;
...
static void maxrpo(Blk* hd, Blk* b)
{
    if (hd->loop < (int)b->id)
        hd->loop = b->id;
}
...
void coalesce(Fn* fn)
{
    ...
    if (debug['M']) {
        ...
        if (n) {
            fputs("\tkill [", stderr);
            for (m = 0; m < n; m++)
                fprintf(stderr, " %%%s", fn->tmp[stk[m]].name);
            fputs(" ]\n", stderr);
        }
    }
    while (n--) {
    ...
    if (debug['M']) {
        for (s0 = sl; s0 < &sl[nsl]; s0++) {
            ...
            for (s = s0; s < &sl[nsl]; s++) {
                ...
                if (s->r.b)
                    fprintf(stderr, "[%d,%d)", s->r.a - ip, s->r.b - ip);
                else
                    fputs("{}", stderr);
            }
            fputs(" ]\n", stderr);
