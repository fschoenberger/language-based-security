...
static PState parseline(PState ps)
{
    ...
    case Tblit:
        ...
        r = INT(c->bits.i);
        if (c->type != CBits || rsval(r) < 0 || rsval(r) != c->bits.i)
...
static int usecheck(Ref r, int k, Fn* fn) { return rtype(r) != RTmp || fn->tmp[r.val].cls == k || (fn->tmp[r.val].cls == Kl && k == Kw); }
...
static void typecheck(Fn* fn)
{
    ...
    for (b = fn->start; b; b = b->link) {
        ...
        for (p = b->phi; p; p = p->link) {
            ...
            for (n = 0; n < p->narg; n++) {
                ...
                if (!usecheck(p->arg[n], k, fn))
                    err("invalid type for operand %%%s in phi %%%s", fn->tmp[p->arg[n].val].name, t->name);
        ...
        for (i = b->ins; i < &b->ins[b->nins]; i++)
            for (n = 0; n < 2; n++) {
                ...
                if (rtype(r) == RType)
                    continue;
                if (rtype(r) != -1 && k == Kx)
                    err("no %s operand expected in %s", n == 1 ? "second" : "first", optab[i->op].name);
                if (rtype(r) == -1 && k != Kx)
                    err("missing %s operand in %s", n == 1 ? "second" : "first", optab[i->op].name);
                if (!usecheck(r, k, fn))
                    err("invalid type for %s operand %%%s in %s", n == 1 ? "second" : "first", t->name, optab[i->op].name);
            }
        r = b->jmp.arg;
        if (isret(b->jmp.type)) {
            if (b->jmp.type == Jretc)
        ...
        JErr:
            err("invalid type for jump argument %%%s in block @%s", fn->tmp[r.val].name, b->name);
...
static void parsefields(Field* fld, Typ* ty, int t)
{
    ...
    while (t != Trbrace) {
        ...
        switch (t) {
        default:
            err("invalid type member specifier");
        case Td:
            type = Fd;
            s = 8;
            a = 3;
...
static void parsedat(void cb(Dat*), Lnk* lnk)
{
    ...
    if (t == Talign) {
        if (nextnl() != Tint)
            err("alignment expected");
        if (tokval.num <= 0 || tokval.num > CHAR_MAX || (tokval.num & (tokval.num - 1)) != 0)
    ...
    for (;;) {
        switch (nextnl()) {
        default:
            err("invalid size specifier %c in data", tokval.chr);
        case Trbrace:
            goto Done;
        case Tl:
            d.type = DL;
...
void parse(FILE* f, char* path, void dbgfile(char*), void data(Dat*), void func(Fn*))
{
    Lnk lnk;
    uint n;
