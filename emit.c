...
void emitlnk(char* n, Lnk* l, int s, FILE* f)
{
    ...
    if (T.apple && l->thread) {
        ...
        sfx = "$tlv$init";
        fputs(".section __DATA,__thread_vars,"
...
void emitfnlnk(char* n, Lnk* l, FILE* f) { emitlnk(n, l, SecText, f); }

void emitdat(Dat* d, FILE* f)
{
    static char* dtoa[] = { [DB] = "\t.byte", [DH] = "\t.short", [DW] = "\t.int", [DL] = "\t.quad" };
    static int64_t zero;
    char* p;
    ...
    case DEnd:
        if (d->lnk->common) {
            ...
            p = d->name[0] == '"' ? "" : T.assym;
            fprintf(f, ".comm %s%s,%" PRId64, p, d->name, zero);
            if (d->lnk->align)
    ...
    default:
        ...
        if (d->isstr) {
            ...
            fprintf(f, "%s %s%s%+" PRId64 "\n", dtoa[d->type], p, d->u.ref.name, d->u.ref.off);
        } else {
            fprintf(f, "%s %" PRId64 "\n", dtoa[d->type], d->u.num);
        }
        break;
...
int stashbits(void* bits, int size)
{
    Asmbits **pb, *b;
    int i;
...
static void emitfin(FILE* f, char* sec[3])
{
    Asmbits* b;
    char* p;
    int lg, i;
...
void elf_emitfnfin(char* fn, FILE* f)
{
    fprintf(f, ".type %s, @function\n", fn);
    fprintf(f, ".size %s, .-%s\n", fn, fn);
}

void macho_emitfin(FILE* f)
{
    static char* sec[3] = {
...
void emitdbgfile(char* fn, FILE* f)
{
    uint32_t id;
    uint n;
...
void emitdbgloc(uint line, uint col, FILE* f)
{
    if (col != 0)
        fprintf(f, "\t.loc %u %u %u\n", curfile, line, col);
    else
