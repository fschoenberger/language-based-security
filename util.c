...
void strf(char str[NString], char* s, ...)
{
    va_list ap;
...
int symeq(Sym s0, Sym s1) { return s0.type == s1.type && s0.id == s1.id; }

Ref newcon(Con* c0, Fn* fn)
{
    Con* c1;
    int i;
    ...
    for (i = 1; i < fn->ncon; i++) {
        c1 = &fn->con[i];
        if (c0->type == c1->type && symeq(c0->sym, c1->sym) && c0->bits.i == c1->bits.i)
            return CON(i);
    }
...
int addcon(Con* c0, Con* c1, int m)
{
...
void salloc(Ref rt, Ref rs, Fn* fn)
{
    Ref r0, r1;
    int64_t sz;
    ...
    if (rtype(rs) == RCon) {
        ...
        if (fn->tmp[rs.val].slot != -1)
            err("unlikely alloc argument %%%s for %%%s", fn->tmp[rs.val].name, fn->tmp[rt.val].name);
    }
}
...
void runmatch(uchar* code, Num* tn, Ref ref, Ref* var)
{
    Ref stkbuf[20], *stk;
    uchar *s, *pc;
    int bc, i;
