...
static int* hint(int t) { return &tmp[phicls(t, tmp)].hint.r; }

static void sethint(int t, int r)
{
    Tmp* p;
...
static void rcopy(RMap* ma, RMap* mb)
{
...
static void radd(RMap* m, int t, int r)
{
    ...
    assert(((T.gpr0 <= r && r < T.gpr0 + T.ngpr) || (T.fpr0 <= r && r < T.fpr0 + T.nfpr)) && "invalid register");
...
static Ref ralloctry(RMap* m, int t, int try)
{
    bits regs;
    int h, r, r0, r1;
...
static inline Ref ralloc(RMap* m, int t) { return ralloctry(m, t, 0); }

static int rfree(RMap* m, int t)
{
    int i, r;
...
static void mdump(RMap* m)
{
    int i;

    for (i = 0; i < m->n; i++)
        if (m->t[i] >= Tmp0)
            fprintf(stderr, " (%s, R%d)", tmp[m->t[i]].name, m->r[i]);
    fprintf(stderr, "\n");
}

static void pmadd(Ref src, Ref dst, int k)
{
    if (npm == Tmp0)
...
static int pmrec(enum PMStat* status, int i, int* k)
{
    int j, c;
...
static void move(int r, Ref to, RMap* m)
{
    int n, t, r1;
...
static int regcpy(Ins* i) { return i->op == Ocopy && isreg(i->arg[0]); }

static Ins* dopm(Blk* b, Ins* i, RMap* m)
{
    RMap m0;
    int n, r, r1, t, s;
    Ins *i1, *ip;
...
static int prio1(Ref r1, Ref r2)
{
...
static void insert(Ref* r, Ref** rs, int p)
{
    int i;
...
static void doblk(Blk* b, RMap* cur)
{
    int t, x, r, rf, rt, nr;
    bits rs;
    Ins *i, *i1;
    ...
    for (i1 = &b->ins[b->nins]; i1 != b->ins;) {
        ...
        if (rf != -1 && (t = cur->w[rf]) != 0)
            if (!bshas(cur->b, rf) && *hint(t) == rf && (rt = rfree(cur, t)) != -1) {
                tmp[t].visit = -1;
                ralloc(cur, t);
...
static int carve(const void* a, const void* b)
{
    Blk *ba, *bb;
...
static int prio2(int t1, int t2)
{
...
void rega(Fn* fn)
{
    ...
    for (s = fn->start; s; s = s->link) {
        ...
        for (p = s->phi; p; p = p->link) {
            if (rtype(p->to) != RTmp || (r = rfind(m, p->to.val)) == -1)
                continue;
            for (u = 0; u < p->narg; u++) {
                b = p->blk[u];
                src = p->arg[u];
